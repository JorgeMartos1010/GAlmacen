...
<div>		
	...
	$tabla="productos";// Escribir entre comillas el nombre de la tabla a listar

	// Establecemos la sentencia SQL de la Consulta a realizar
	$sentencia="select * from $tabla order by nombreproducto ";
	...
	// Formulario para enviar los datos
		echo '<form name="alta" method="POST" action="AProductos.php">';
		//echo '<input type="hidden" name="codigo" value="'.$registro[0].'">';
		echo '<input type="image" src="estilos/mas.jpg" height="20px" title="Alta de producto.">';
		echo '</form>';
	echo "</th>";
	echo "<th></th>";
	echo "</tr>";
	...
	while ($registro = mysqli_fetch_row($resultado)){
		...
		foreach ($registro as $fila){
			...
			if ($i==5){
				echo "<td> <img src='../imagenes/$registro[$i]' height='30'></td>";
			} else{
		...
		echo "<td align='center'>";	
			// Para ver solo un registro.
				// Formulario para enviar los datos
				echo '<form name="veruno" method="POST" action="VerProducto.php">';
				echo '<input type="hidden" name="id" value="'.$registro[0].'">';
				echo '<input type="hidden" name="producto" value="'.$registro[1].'">';
				echo '<input type="image" src="estilos/ver.jpg" title="Ver datos del producto... '.$registro[1].'">';
				echo '</form>';	
			echo "</td><td align='center'>";	
			// Para borrar un registro.
			// Formulario para enviar los datos
				echo '<form name="borraruno" method="POST" action="BorraProducto.php">';
				echo '<input type="hidden" name="id" value="'.$registro[0].'">';
				echo '<input type="image" src="estilos/papelera.jpg" title="Borrar el producto... '.$registro[1].'">';
				echo '</form>';
			echo "</td><td align='center'>";		
			// Para modificar un registro.
			// Formulario para enviar los datos
				echo '<form name="editaruno" method="POST" action="ModificaProducto.php">';
				echo '<input type="hidden" name="id" value="'.$registro[0].'">';
				echo '<input type="image" src="estilos/lapiz.jpg" title="Cambiar datos del producto... '.$registro[1].'">';
				echo '</form>';	
	...
	echo '	  <a class="page-link" href="GProductos.php?pagina='.$anterior.'">&laquo;</a>';
	echo '	</li>';
	for ($i=1;$i<=$numpag;$i++)
	{
		if($i==$pagina){
				echo '	<li class="page-item active">';
				echo '	  <a class="page-link"href="GProductos.php?pagina='.$i.'">'.$i.'</a>';
				echo '	</li>';						
			}
			else{
	...
	echo '	<li class="page-item ">';
	echo '	  <a class="page-link" href="GProductos.php?pagina='.$siguiente.'">&raquo;</a>';
	echo '	</li>';
	echo '  </ul>';
	echo "</div>";
	// --- Fin paginación ---
