...
<div>		
	...
	$sentencia="select * from $tabla order by nombreprov ";
	...
	// Formulario para enviar los datos
		echo '<form name="alta" method="POST" action="AProveedores-B.php">';
		//echo '<input type="hidden" name="codigo" value="'.$registro[0].'">';
		echo '<input type="image" src="estilos/mas.jpg" height="20px" title="Alta de proveedores.">';
		echo '</form>';
	echo "</th>";
	echo "<th></th>";
	echo "</tr>";
	...
	while ($registro = mysqli_fetch_row($resultado)){
		...
		echo "<td align='center'>";	
			// Para ver solo un registro.
				// Formulario para enviar los datos
				echo '<form name="veruno" method="POST" action="VerProveedor.php">';
				echo '<input type="hidden" name="id" value="'.$registro[0].'">';
				echo '<input type="hidden" name="proveedor" value="'.$registro[1].'">';
				echo '<input type="image" src="estilos/ver.jpg" title="Ver datos del proveedor... '.$registro[1].'">';
				echo '</form>';	
			echo "</td><td align='center'>";	
			// Para borrar un registro.
			// Formulario para enviar los datos
				echo '<form name="borraruno" method="POST" action="BorraProveedor.php">';
				echo '<input type="hidden" name="id" value="'.$registro[0].'">';
				echo '<input type="image" src="estilos/papelera.jpg" title="Borrar el proveedor... '.$registro[1].'">';
				echo '</form>';
			echo "</td><td align='center'>";		
			// Para modificar un registro.
			// Formulario para enviar los datos
				echo '<form name="editaruno" method="POST" action="ModificaProveedor.php">';
				echo '<input type="hidden" name="id" value="'.$registro[0].'">';
				echo '<input type="image" src="estilos/lapiz.jpg" title="Cambiar datos del proveedor... '.$registro[1].'">';
				echo '</form>';	
	...
	echo '	  <a class="page-link" href="GProveedores.php?pagina='.$anterior.'">&laquo;</a>';
	echo '	</li>';
	for ($i=1;$i<=$numpag;$i++)
	{
		if($i==$pagina){
				echo '	<li class="page-item active">';
				echo '	  <a class="page-link"href="GProveedores.php?pagina='.$i.'">'.$i.'</a>';
				echo '	</li>';						
			}
			else{
	...
	echo '	<li class="page-item ">';
	echo '	  <a class="page-link" href="GProveedores.php?pagina='.$siguiente.'">&raquo;</a>';
	echo '	</li>';
	echo '  </ul>';
	echo "</div>";
	// --- Fin paginación ---
